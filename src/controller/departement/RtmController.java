 ...
 * @author User
 */
public class RtmController implements Initializable {
    @FXML
...
//        });
 ...
 public void getData() {
        ...
        try {
            PreparedStatement ps = con.prepareStatement("SELECT * FROM `v_materiel` WHERE `id_Dep`=2");
  ...
  public void rechercher() {
        if (t_search.getText().equals("")) {
            ...
            table.getItems().clear();
            try {
                PreparedStatement ps = con.prepareStatement("SELECT * FROM `v_materiel` WHERE `id_Dep`=2 AND " + cmbFilter.getValue().toString() + " like ?");
